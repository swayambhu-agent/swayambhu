...
| `contact_index:{platform}:{userId}` | JSON string (contact slug) | `kernel.js:resolveContact()` (cache miss), `dashboard-api:POST /contacts` | `kernel.js:resolveContact()` | No (auto-created) | System (prefix `contact_index:`) + operator-only |
