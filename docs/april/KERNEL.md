...
Userspace (`userspace.js`) receives the kernel interface (`K`) and raw
infrastructure inputs. It decides everything: whether to run an act
cycle, whether to dispatch deep-reflect, how to structure the work.
Act and DR dispatch are independent concerns — either can fail without
affecting the other.
...
The kernel provides infrastructure. Userspace provides policy.
If a piece of logic decides *what* the agent should do, it belongs in
userspace. If it enforces *what the agent cannot do*, it belongs in the
kernel.

**Userspace files:** `userspace.js` (entry point), `act.js`, `eval.js`,
`memory.js`, `reflect.js`, `prompts/*.md`, `tools/*.js`, `providers/*.js`,
`channels/*.js`. All mutable — the agent can propose changes via code staging.
