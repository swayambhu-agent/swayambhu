...
- it frames identity as gated behind prerequisites that must be satisfied
  before implementation or activation
- current code already contains identity-specific runtime seams in
  `kernel.js`, `userspace.js`, and `prompts/deep_reflect.md`
- it should therefore not be treated as the current source of truth for
  rollout status or integration location

Keep it as design intent for the identity layer, but verify all implementation
claims against current code before relying on it.
...
- provide lightweight environment breadth context to planning
  - accessible roots
  - explored paths
  - durable discovered-project rehydration into `open_surfaces`
...
`open_surfaces` must not be one-shot filesystem memory only. If the agent
discovers a legitimate adjacent project surface and writes it into durable
state (for example `workspace:discovered_projects`), later sessions should
rehydrate that surface back into planning context until it is actually
exhausted, explicitly rejected, or superseded. Otherwise the planner can
correctly conclude that all surfaces are exhausted even though durable
discovery state already says otherwise.
