...
tool) when `config:defaults.jobs.base_url` is configured. Results are stored in
KV and applied by the next session. Falls back to in-Worker execution when akash
is not configured. The M/D operator prompt is stored as `prompt:deep_reflect`.
