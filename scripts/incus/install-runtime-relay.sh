...
ENV_FILE="$ENV_DIR/email-gateway.env"
SERVICE_NAME="swayambhu-email-gateway.service"
SYNC_REPO=1
START_IF_READY=1

usage() {
  cat <<EOF
Install the email relay into the runtime container.

Usage:
  bash scripts/incus/install-runtime-relay.sh [options]
...
ExecStart=/usr/bin/node $REMOTE_REPO/services/email-gateway.mjs
Restart=on-failure
RestartSec=5
...
env_ready() {
  ...
  "$CONTAINER_CLI" exec "$container" -- bash -lc "
    awk -F= '
      BEGIN { bad = 0 }
      /^EMAIL_RELAY_SECRET=/{ if (\$2 == \"\" || \$2 == \"REPLACE_ME\") bad = 1 }
      /^GMAIL_USER=/{ if (\$2 == \"\" || \$2 == \"REPLACE_ME\") bad = 1 }
      /^GMAIL_APP_PASSWORD=/{ if (\$2 == \"\" || \$2 == \"REPLACE_ME\") bad = 1 }
      END { exit bad }
    ' '$ENV_FILE'
  "
}
...
if [ "$START_IF_READY" -eq 1 ] && env_ready "$RUNTIME_NAME"; then
  ...
  "
  echo "Relay installed and started in $RUNTIME_NAME."
else
  echo "Relay files installed in $RUNTIME_NAME, but the service was not started."
