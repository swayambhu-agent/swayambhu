...
export async function maybeSendProgressEmail(enabled, subject, lines) {
  if (!enabled) return;
  try {
    const { sendEmail } = await import("../../scripts/operator/dev-loop/comms.mjs");
...
export async function main(argv = process.argv.slice(2)) {
  ...
  const [{ initState, saveRun }, { ensureServices }, { runObserve }, { runClassify }, { cleanRemoteComputeSurfaces }] = await Promise.all([
    import("../../scripts/operator/dev-loop/state.mjs"),
    import("../../scripts/operator/dev-loop/services.mjs"),
    import("../../scripts/operator/dev-loop/observe.mjs"),
    import("../../scripts/operator/dev-loop/classify.mjs"),
    import("../../scripts/operator/dev-loop/remote-compute.mjs"),
  ]);
