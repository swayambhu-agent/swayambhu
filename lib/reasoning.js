...
export async function loadReasoningArtifacts(fsImpl = fs) {
  let dirEntries = [];
  try {
    dirEntries = await fsImpl.readdir(REASONING_DIR, { withFileTypes: true });
  } catch {
    return [];
  }
  ...
  for (const entry of dirEntries) {
    ...
    const markdown = await fsImpl.readFile(join(REASONING_DIR, entry.name), "utf8");
