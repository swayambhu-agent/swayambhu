...
function splitTarPath(name) {
  if (typeof name !== "string" || !name) {
    throw new Error("tar entry name is required");
  }
  ...
  for (let i = 0; i < name.length; i += 1) {
    if (name[i] === "/") slashIndexes.push(i);
  }

  for (let i = slashIndexes.length - 1; i >= 0; i -= 1) {
    const cut = slashIndexes[i];
    const prefix = name.slice(0, cut);
    const basename = name.slice(cut + 1);
    const prefixBytes = new TextEncoder().encode(prefix);
    const basenameBytes = new TextEncoder().encode(basename);
    if (basenameBytes.length <= 100 && prefixBytes.length <= 155) {
      return { prefix, basename };
    }
  }

  throw new Error(`tar entry path too long: ${name}`);
}
...
export function buildTar(files) {
  ...
  for (const file of files) {
    ...
    const { prefix, basename } = splitTarPath(file.name);

    encodeString(header, 0, basename, 100);
    ...
    encodeString(header, 345, prefix, 155);    // prefix for long names
