...
export function resolveRequestContact(request) {
  ...
  if (request.contact) return request.contact;
  if (request.requester?.type === "contact" && request.requester.id) return request.requester.id;
  return null;
}
...
export async function applyRequestUpdate({
  ...
  const contact = resolveRequestContact(existing);
  await emitEvent("session_response", {
    contact,
    requester: existing.requester || null,
    ref: requestKey,
    status,
  });
