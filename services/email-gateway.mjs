...
async function sendViaSMTP({ to, subject, body, in_reply_to }) {
  ...
  await new Promise((resolve, reject) => {
    plainSocket.on("connect", resolve);
    plainSocket.on("error", reject);
    setTimeout(() => reject(new Error("SMTP connect timeout")), SMTP_TIMEOUT);
  });
  ...
  try {
    ...
    await new Promise((resolve, reject) => {
      ...
      setTimeout(() => reject(new Error("TLS timeout")), SMTP_TIMEOUT);
    });
...
const server = createServer(async (req, res) => {
  ...
  if (req.method === "POST" && req.url === "/send-email") {
    ...
    try {
      ...
      return json(res, 200, { ok: true, message_id: result.message_id });
    } catch (err) {
